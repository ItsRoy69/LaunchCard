import { PALETTES, SIZES, displayHandle, displayUrl, resolvePalette } from "./catalog";
import { FONT, setActiveFontPair } from "./fonts";
...
function kicker(c: Ctx, x: number, y: number, size: number) {
  c.ctx.fillStyle = c.pal.accent || c.pal.muted;
...
function editorial(c: Ctx) {
  ...
  const { lines, size } = paragraph(
    ctx, name, titleMaxW, maxNameSize, 22 * k, c.mode === "portrait" ? 4 : 2,
    (s) => `400 ${s}px ${FONT.serif}`, true,
  );
  let y = pad + 36 * k;
  ctx.fillStyle = pal.fg;
  ...
  if (tag) {
    const body = paragraph(
      ctx, tag, c.mode === "portrait" ? w - pad * 2 : (w - pad * 2) * 0.78,
      22 * k, 14 * k, c.mode === "portrait" ? 6 : 3, (s) => `400 ${s}px ${FONT.sans}`,
    );
    ctx.fillStyle = pal.muted;
  ...
  drawStatsRow(c, pad, h - pad - 52 * k, w - pad * 2, 22 * k);
...
function flex(c: Ctx) {
  ...
  if (c.mode === "banner") {
    ...
    const hx = w * 0.52;
    const hs = fitOne(ctx, heroText, w - hx - pad, h * 0.5, 18, (s) => `800 ${s}px ${FONT.poster}`);
    ctx.fillStyle = pal.fg;
  ...
  ctx.fillText(ellipsize(ctx, displayHandle(c.doc.handle) || displayUrl(c.doc.url), nameMax), nx, pad + 26 * k);
  ...
  ctx.fillStyle = pal.fg;
  ctx.font = `800 ${hs}px ${FONT.poster}`;
  ctx.fillText(heroText, pad, hy);
  const labelY = hy + inkDepth(ctx, heroText, hs) + 10 * k;
  ctx.fillStyle = pal.muted;
  ctx.font = `500 ${13 * k}px ${FONT.sans}`;
  ctx.fillText(ellipsize(ctx, heroLabel, heroMaxW), pad, labelY);
  const tag = c.doc.tagline.trim();
  if (tag && c.mode !== "portrait") {
    const colX = w * 0.52;
    const body = paragraph(ctx, tag, w - colX - pad, 20 * k, 13 * k, 5, (s) => `400 ${s}px ${FONT.sans}`);
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${body.size}px ${FONT.sans}`;
    let ty = hy + hs * 0.12;
    for (const line of body.lines) {
  ...
  if (rest.length) drawStatsRow(c, pad, h - pad - 48 * k, w - pad * 2, 18 * k, rest);
}
...
function split(c: Ctx) {
  ...
  const left = c.mode === "portrait" ? w : w * 0.38;
  if (c.mode === "portrait") {
    ctx.fillStyle = pal.fg;
  ...
  ctx.fillStyle = pal.bg;
  const maxW = (c.mode === "portrait" ? w : left) - pad * 2;
  const { lines, size } = paragraph(ctx, name, maxW, 44 * k, 22 * k, 4, (s) => `400 ${s}px ${FONT.serif}`);
  let y = pad + mark + 28 * k;
  ctx.font = `400 ${size}px ${FONT.serif}`;
  for (const line of lines) {
    ctx.fillText(line, pad, y);
...
function terminal(c: Ctx) {
  ...
  [pal.muted, pal.muted, pal.fg].forEach((col, i) => {
    ctx.fillStyle = col;
    ctx.beginPath();
  ...
  const x = inset + 18 * k;
  let y = inset + barH + 16 * k;
  const name = title(c.doc);
  ctx.fillStyle = pal.muted;
  ctx.font = `400 ${13 * k}px ${FONT.mono}`;
  ctx.fillText("$ launch --product", x, y);
  y += 22 * k;
  ctx.fillStyle = pal.fg;
  ctx.font = `600 ${18 * k}px ${FONT.mono}`;
  ctx.fillText(ellipsize(ctx, name, w - inset * 2 - 36 * k), x, y);
  y += 28 * k;
  const tag = c.doc.tagline.trim();
  if (tag) {
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${14 * k}px ${FONT.mono}`;
    const body = paragraph(ctx, tag, w - inset * 2 - 36 * k, 14 * k, 12 * k, 4, (s) => `400 ${s}px ${FONT.mono}`);
    for (const line of body.lines) {
      ctx.fillText(line, x, y);
      y += body.lh;
    }
  }
  drawStatsRow(c, x, h - inset - 48 * k, w - inset * 2 - 36 * k, 16 * k);
}
...
function poster(c: Ctx) {
  ...
  typeReset(ctx);
  const maxW = w - pad * 2;
  const maxSize = c.mode === "banner" ? h * 0.55 : c.mode === "portrait" ? w * 0.2 : h * 0.32;
  const ns = fitOne(ctx, name, maxW, maxSize, 24 * k, (s) => `800 ${s}px ${FONT.poster}`);
  ctx.fillStyle = pal.fg;
  ctx.font = `800 ${ns}px ${FONT.poster}`;
  if (c.mode === "banner") {
    ctx.textBaseline = "middle";
    ctx.fillText(ellipsize(ctx, name, maxW), pad, h / 2);
    ctx.textBaseline = "top";
    return;
  }
  ctx.fillText(ellipsize(ctx, name, maxW), pad, pad + 20 * k);
  const tag = c.doc.tagline.trim();
  if (tag) {
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${16 * k}px ${FONT.sans}`;
    ctx.fillText(ellipsize(ctx, tag, maxW), pad, h - pad - 24 * k);
  }
}
...
function quiet(c: Ctx) {
  ...
  kicker(c, inner, c.mode === "banner" ? h * 0.22 : pad, 11 * k);
  const maxW = w - inner * 2;
  const ns = fitOne(ctx, name, maxW, c.mode === "banner" ? h * 0.28 : 42 * k, 20 * k, (s) => `400 ${s}px ${FONT.serif}`);
  ctx.fillStyle = pal.fg;
  ctx.font = `400 ${ns}px ${FONT.serif}`;
  const y = c.mode === "banner" ? h * 0.4 : pad + 36 * k;
  if (c.mode === "banner") {
    ctx.textBaseline = "middle";
    ctx.fillText(ellipsize(ctx, name, maxW), inner, y);
    ctx.textBaseline = "top";
  } else {
    ctx.fillText(ellipsize(ctx, name, maxW), inner, y);
  }
  ...
  if (tag && c.mode !== "banner") {
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${15 * k}px ${FONT.sans}`;
    ctx.fillText(ellipsize(ctx, tag, maxW * 0.8), inner, y + ns + 20 * k);
  }
}
...
function ledger(c: Ctx) {
  ...
  const name = title(c.doc);
  const mid = w * 0.5;
  hairline(ctx, mid, pad, 1, pal);
  ctx.fillStyle = pal.rule;
  ctx.fillRect(mid, pad, 1, h - pad * 2);
  ctx.fillStyle = pal.fg;
  ctx.font = `600 ${20 * k}px ${FONT.sans}`;
  ctx.fillText(ellipsize(ctx, name, mid - pad * 2), pad, pad);
  const tag = c.doc.tagline.trim();
  if (tag) {
    const body = paragraph(ctx, tag, mid - pad * 2, 16 * k, 12 * k, 8, (s) => `400 ${s}px ${FONT.sans}`);
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${body.size}px ${FONT.sans}`;
    let y = pad + 36 * k;
    for (const line of body.lines) {
  ...
  drawStatsStack(c, mid + pad, pad, mid - pad * 2, 20 * k);
}
...
function frame(c: Ctx) {
  ...
  const cy = imgH + 22 * k;
  const mark = 36 * k;
  if (c.mode !== "portrait") drawMark(ctx, pad, imgH + (caption - mark) / 2, mark, pal, c.logo, name);
...
function signal(c: Ctx) {
  const { ctx, w, h, pad, pal, k } = c;
  const name = title(c.doc);
  const bar = Math.max(6, Math.round(10 * k));
  ctx.fillStyle = pal.accent;
  ctx.fillRect(0, 0, bar, h);
  const mark = c.mode === "banner" ? Math.min(h * 0.5, 48 * k) : 48 * k;
  const x0 = pad + bar + 8 * k;
  drawMark(ctx, x0, c.mode === "banner" ? (h - mark) / 2 : pad, mark, pal, c.logo, name);
  typeReset(ctx);
  if (c.mode === "banner") {
    const tx = x0 + mark + 14 * k;
    ctx.fillStyle = pal.fg;
    ctx.font = `600 ${h * 0.22}px ${FONT.sans}`;
    ctx.textBaseline = "middle";
    ctx.fillText(ellipsize(ctx, name, w * 0.35), tx, h * 0.4);
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${h * 0.1}px ${FONT.sans}`;
    ctx.fillText(ellipsize(ctx, c.doc.tagline.trim(), w * 0.35), tx, h * 0.62);
    ctx.textBaseline = "top";
    drawStatsRow(c, w * 0.58, h * 0.3, w * 0.36, h * 0.16);
    return;
  }
  const tx = x0 + mark + 14 * k;
  ctx.fillStyle = pal.fg;
  ctx.font = `600 ${16 * k}px ${FONT.sans}`;
  ctx.fillText(ellipsize(ctx, name, w - tx - pad), tx, pad + 6 * k);
  ctx.fillStyle = pal.muted;
  ctx.font = `400 ${13 * k}px ${FONT.sans}`;
  ctx.fillText(ellipsize(ctx, displayHandle(c.doc.handle) || displayUrl(c.doc.url), w - tx - pad), tx, pad + 28 * k);
  const tag = c.doc.tagline.trim();
  let y = pad + mark + 36 * k;
  if (tag) {
    const body = paragraph(ctx, tag, w - x0 - pad, 24 * k, 14 * k, c.mode === "portrait" ? 8 : 4, (s) => `400 ${s}px ${FONT.sans}`);
    ctx.fillStyle = pal.fg;
    ctx.font = `400 ${body.size}px ${FONT.sans}`;
    for (const line of body.lines) {
  ...
  drawStatsRow(c, x0, h - pad - 48 * k, w - x0 - pad, 20 * k);
}

function marquee(c: Ctx) {
  const { ctx, w, h, pad, pal, k } = c;
  const name = title(c.doc);
  typeReset(ctx);
  const maxW = w - pad * 2;
  const maxSize = c.mode === "banner" ? h * 0.62 : c.mode === "portrait" ? w * 0.22 : h * 0.28;
  const ns = fitOne(ctx, name, maxW, maxSize, 28 * k, (s) => `700 ${s}px ${FONT.poster}`);
  ctx.fillStyle = pal.fg;
  ctx.font = `700 ${ns}px ${FONT.poster}`;
  if (c.mode === "banner") {
    ctx.textBaseline = "middle";
    ctx.fillText(ellipsize(ctx, name, maxW), pad, h * 0.42);
    ctx.textBaseline = "top";
    ctx.fillStyle = pal.accent;
    ctx.fillRect(pad, h * 0.72, Math.min(maxW, 120 * k), 3 * k);
    return;
  }
  let y = pad + (c.mode === "portrait" ? 40 * k : 28 * k);
  ctx.fillText(ellipsize(ctx, name, maxW), pad, y);
  y += ns * 1.1;
  ctx.fillStyle = pal.accent;
  ctx.fillRect(pad, y, Math.min(maxW * 0.35, 160 * k), 4 * k);
  y += 28 * k;
  const tag = c.doc.tagline.trim();
  if (tag) {
    const body = paragraph(ctx, tag, maxW * (c.mode === "portrait" ? 1 : 0.7), 20 * k, 13 * k, 5, (s) => `400 ${s}px ${FONT.sans}`);
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${body.size}px ${FONT.sans}`;
    for (const line of body.lines) {
      ctx.fillText(line, pad, y);
  ...
  drawStatsRow(c, pad, h - pad - 56 * k, maxW, 18 * k);
  const meta = metaLine(c);
  if (meta) {
    ctx.fillStyle = pal.muted;
    ctx.font = `400 ${12 * k}px ${FONT.mono}`;
    ctx.fillText(ellipsize(ctx, meta, maxW), pad, h - pad);
  }
}
...
function paint(c: Ctx) {
  ...
  ctx.fillRect(0, 0, w, h);
  typeReset(ctx);
  const fn = TEMPLATES[c.doc.templateId] ?? TEMPLATES.editorial;
  fn(c);
  ctx.restore();
...
export async function renderCard(
  ...
  const pal = resolvePalette(doc.paletteId, doc.accent ?? null);
  setActiveFontPair(doc.fontPairId ?? "classic");
  const scale = opts.scale ?? doc.exportScale ?? 2;
  const w = size.w;
  const h = size.h;
...
export async function cardToBlob(doc: CardDoc, sizeId?: string): Promise<Blob> {
  ...
  await renderCard(canvas, { ...doc, sizeId: sizeId ?? doc.sizeId }, { scale });
